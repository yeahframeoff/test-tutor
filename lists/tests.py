...
class HomePageTest(TestCase):
    ...
    def test_home_page_can_process_a_POST_request(self):
        request = HttpRequest()
        request.method = 'POST'
    ...
    def test_home_page_displays_all_list_items(self):
        Item.objects.create(text='aytem 1')
        Item.objects.create(text='aytem 2')

        request = HttpRequest()
        response = home_page(request)

        self.assertIn('aytem 1', response.content.decode())
        self.assertIn('aytem 2', response.content.decode())
